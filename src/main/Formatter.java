...
/**
 * After using XMLCreator created xmlString doesn't have relevant format at the
 * beginning. Formatting is related to display with use of Stylesheet.xsl.
 * 
 * @author MJazy
 *
 */
...
public class Formatter {
	...
	public String formatString() {
		...
		String basicInformationElementString = "<basicInformation>" + fullNameElementString + birthdateElementString
				+ cityElementString + phoneNumberElementString + pictureLinkElementString + "</basicInformation>";
		...
		List<String> fieldOfStudyContent = prepareSimpleElementContent("<fieldOfStudy>", "</fieldOfStudy>",
				"</string>");
		Element fieldOfStudyElement = new Element("<fieldOfStudy>", "</fieldOfStudy>", fieldOfStudyContent);
		List<String> studyStartDateContent = prepareSimpleElementContent("<studyStartDate>", "</studyStartDate>",
				"</string>");
		Element studyStartDateElement = new Element("<studyStartDate>", "</studyStartDate>", studyStartDateContent);
		List<String> studyEndDateContent = prepareSimpleElementContent("<studyEndDate>", "</studyEndDate>",
				"</string>");
		Element studyEndDateElement = new Element("<studyEndDate>", "</studyEndDate>", studyEndDateContent);

		String educationElementString = prepareComplexElementString("<education>", "</education>", collegeElement,
				fieldOfStudyElement, studyStartDateElement, studyEndDateElement);
		...
		List<String> jobStartDateContent = prepareSimpleElementContent("<jobStartDate>", "</jobStartDate>",
				"</string>");
		...
		String experienceElementString = prepareComplexElementString("<experience>", "</experience>", companyElement,
				postElement, jobStartDateElement, jobEndDateElement);

		List<String> languageNameContent = prepareSimpleElementContent("<languageName>", "</languageName>",
				"</string>");
		Element languageNameElement = new Element("<languageName>", "</languageName>", languageNameContent);
		List<String> languageLevelContent = prepareSimpleElementContent("<languageLevel>", "</languageLevel>",
				"</string>");
		Element languageLevelElement = new Element("<languageLevel>", "</languageLevel>", languageLevelContent);
		String languageElementString = prepareComplexElementString("<language>", "</language>", languageNameElement,
				languageLevelElement);

		// Element skills doesn't have to be additionally formatted. 9 is the number of
		// signs in "</skills>".
		String skillsElementString = xmlString.substring(xmlString.indexOf("<skills>"),
				xmlString.indexOf("</skills>") + 9);

		xmlString = "<CV>" + basicInformationElementString + educationElementString + experienceElementString
				+ languageElementString + skillsElementString + "</CV>";
		return xmlString;

	}

	// Complex - having more than one type of parent element inside.
	private static String prepareComplexElementString(String openingBracket, String closingBracket,
		...
		for (int index = 0; index < simpleElementSize; index++) {
			...
			for (Element simpleElement : simpleElements) {
				complexElementString = complexElementString + simpleElement.getOpeningBracket()
						+ simpleElement.getContent().get(index) + simpleElement.getClosingBracket();
			}
	...
	private List<String> prepareSimpleElementContent(String elementOpeningBracket, String elementClosingBracket,
	...
	private String prepareBasicElementString(String openingBracket, String closingBracket) {
		String basicElementString = xmlString.substring(xmlString.indexOf(openingBracket),
				xmlString.indexOf(closingBracket) + closingBracket.length());
